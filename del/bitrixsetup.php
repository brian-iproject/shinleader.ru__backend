...
$arEditions = array(
	...
	'en' => array(
		...
		array(
			'NAME' => 'Bitrix Intranet Portal',
			'LIST' => array(
				'portal/intranet_portal' => "Office Edition",
				'portal/intranet_extranet_portal' => 'Extranet Edition',
				'portal/big_intranet_portal' => 'Enterprise Edition'
			),
			'PRIVATE_PATH' => array(
				'portal/intranet_portal' => "intranet_portal",
				'portal/intranet_extranet_portal' => 'intranet_extranet_portal',
				'portal/big_intranet_portal' => 'big_intranet_portal'
			)
		)
	),
	'de' => array(
		array(
			'NAME' => 'Bitrix Intranet Portal',
			'LIST' => array(
				'de/de_intranet_portal' => "Office Edition",
				'de/de_intranet_extranet' => 'Extranet Edition',
				'de/de_big_intranet' => 'Enterprise Edition'
			),
			'PRIVATE_PATH' => array(
				'de/de_intranet_portal' => "de_intranet_portal",
				'de/de_intranet_extranet' => 'de_intranet_extranet',
				'de/de_big_intranet' => 'de_big_intranet'
			)
		)
	)
);
...
elseif ($strAction=="LOAD")
{
	...
	if($_REQUEST['licence_type'] == 'src' || $_REQUEST['LICENSE_KEY'])
	{
		...
		@fwrite($fres,'<'.'? $LICENSE_KEY = "'.addslashes($_REQUEST['LICENSE_KEY']).'"; ?>') && fclose($fres);
	}
	else
	{
	...
	if ($_REQUEST['LICENSE_KEY'] && isset($ED['PRIVATE_PATH'][$url]))
		$url = $ED['PRIVATE_PATH'][$url];
	...
	{
		$txt = $status;
		$script = "<script>window.location = \"".$this_script_name."?action=LOAD&edition=".urlencode($_REQUEST['edition'])."&url=".urlencode($url)."&lang=".urlencode(LANG)."&LICENSE_KEY=".urlencode($_REQUEST["LICENSE_KEY"])."&action_next=".urlencode($_REQUEST["action_next"])."&xz=".rand(0, 32000)."\";</script>\n";
	}
	else
	{
		$txt = $status;
		$script = "<script>window.location = \"".$this_script_name."?action=UNPACK&by_step=Y&filename=".urlencode(basename($strRequestedUrl))."&xz=".rand(0, 32000)."\";</script>\n";
	}
...
//	$iNumDistrFiles = 8000;
	...
	if ($tres)
	{
		if (!$oArchiver->bFinish)
			$script = "<script>window.location = \"".$this_script_name."?action=UNPACK&filename=".urlencode(basename($oArchiver->_strArchiveName))."&by_step=Y&seek=".$oArchiver->iCurPos."\";</script>\n";
		else // finish
		{
			...
			else
			{
				...
				$script = "<script>window.location = \"/".$strInstFile."\";</script>";
			}
		}
	}
	else
	{
		...
		if (count($arErrors)>0)
		{
			if ($ft = fopen($_SERVER["DOCUMENT_ROOT"]."/".$this_script_name.".log", "wb"))
			{
				...
				fclose($ft);
				#$script = "<script>window.location = \"".$this_script_name."?action=LOG&xz=".rand(0, 32000)."\";</script>";
			}
		}
	}
