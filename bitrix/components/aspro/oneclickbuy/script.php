...
if($bAllBasketBuy){
	...
	if(isset($_POST["OFFER_PROPS"])){
		$arSkuProps=unserialize($_POST["OFFER_PROPS"]);
		if($arSkuProps){
			...
			if($intProductIBlockID == $iblockID){
				...
				$arProps = CIBlockPriceTools::GetOfferProperties($_POST['ELEMENT_ID'], $iblockID, $arSkuProps);
			}
		}
	}
	...
	/*if(is_array($arOrderDat) && array_key_exists("ORDER_PRICE", $arOrderDat)){
		...
		$arError = CTires::checkAllowDelivery($arOrderDat["ORDER_PRICE"], $arOrderDat["CURRENCY"]);
