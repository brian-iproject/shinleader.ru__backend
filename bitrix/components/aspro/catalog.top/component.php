...
if((IntVal($arParams["IBLOCK_ID1"]) > 0)||(IntVal($arParams["IBLOCK_ID2"]) > 0)||(IntVal($arParams["IBLOCK_ID3"]) > 0)||(IntVal($arParams["IBLOCK_ID4"]) > 0))
{
	...
	if (IntVal($arParams["IBLOCK_ID1"]) > 0) $arIblocksFilter[] = $arParams["IBLOCK_ID1"];
	if (IntVal($arParams["IBLOCK_ID2"]) > 0) $arIblocksFilter[] = $arParams["IBLOCK_ID2"];
	if (IntVal($arParams["IBLOCK_ID3"]) > 0) $arIblocksFilter[] = $arParams["IBLOCK_ID3"];
	if (IntVal($arParams["IBLOCK_ID4"]) > 0) $arIblocksFilter[] = $arParams["IBLOCK_ID4"];
}
if ($arIblocksFilter) $arParams["IBLOCK_ID"] = $arIblocksFilter;
//added -->
	...
	if (($action == "ADD2BASKET" || $action == "BUY") && $productID > 0)
	{
		...
		$rsItems = CIBlockElement::GetList(array(), array('ID' => $productID), false, false, array('ID', 'IBLOCK_ID'));
			if ($arItem = $rsItems->Fetch())
			{
				...
				if (in_array($arItem['IBLOCK_ID'], $arParams["IBLOCK_ID"]))
				{
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	************************************/
	...
	//<!--added
	if ($arIblocksFilter) $arrFilter["IBLOCK_ID"] = $arIblocksFilter;
	//added-->
