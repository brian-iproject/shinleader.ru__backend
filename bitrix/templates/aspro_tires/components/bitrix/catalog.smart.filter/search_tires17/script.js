...
JCSmartFilter.prototype.filterCatalog = function (url, set_disabled){
	// add to url filter params by car
	var carFilter = '';
	var car = $('#CAR').val();
	if(car.length){
		...
		if(model.length){
			...
			if(year.length){
				...
				if(modification.length){
					if(url.indexOf('modification') == -1){
						carFilter += '&modification=' + modification;
						var tyre_type = $('input[name="tyre_type"]:checked').val();
						if(tyre_type.length){
							if(url.indexOf('tyre_type') == -1){
								carFilter += '&tyre_type=' + tyre_type;
							}
						}
					}
				}
