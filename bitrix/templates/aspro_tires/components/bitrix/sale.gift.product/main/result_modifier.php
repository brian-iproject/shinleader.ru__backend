	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			...
			foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arAddToBasketData = CKShop::GetAddToBasketArray($arOffer, 1, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);
				...
				$arOneRow = array(
					...
					'AVAILIABLE' => CKShop::GetQuantityArray($arOffer['CATALOG_QUANTITY']),
