...
<?
	...
	else
	{
		...
		{
			if(strlen($arResult["REDIRECT_URL"]) > 0){?><script type="text/javascript">window.top.location.href='<?=CUtil::JSEscape($arResult["REDIRECT_URL"])?>';</script><?die();}
		...
		else
		{
			...
			<script type="text/javascript">
				...
				function SetContact(profileId) { BX("profile_change").value = "Y"; submitForm(); }
			</script>
			...
			if(!empty($arResult["ERROR"]) && $arResult["USER_VALS"]["FINAL_STEP"] == "Y")
			{
				...
				?><script type="text/javascript"> top.BX.scrollToNode(top.BX('ORDER_FORM'));</script><?
			}
			...
			{?>
					...
					<!--noindex-->
						<button type="button" class="button_basket gradient checkout"  name="submitbutton" onClick="submitForm('Y');" value="<?=GetMessage("SOA_TEMPL_BUTTON")?>"><?=GetMessage("SOA_TEMPL_BUTTON")?></button>
					<!--/noindex-->
			...
			{
				?><script type="text/javascript"> top.BX('confirmorder').value = 'Y'; top.BX('profile_change').value = 'N'; </script><?
				die();
			}
		}
	}
