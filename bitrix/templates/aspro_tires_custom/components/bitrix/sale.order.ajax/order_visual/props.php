...
function PrintPropsForm($arSource=Array(), $locationTemplate = "popup")
{
	if (!empty($arSource))
	{
		foreach($arSource as $arProperties)
		{
			...
			<tr>
				<td class="name">
					<?= $arProperties["NAME"] ?>:<?if($arProperties["REQUIED_FORMATED"]=="Y"){?><span class="star">*</span><?}?>
				</td>
				<td class="value">
					<?
					...
					{
						?>						
						<input <?if ($arProperties["CODE"]=="PHONE"):?>class="phone" <?elseif(($arProperties["IS_EMAIL"]=="Y")||($arProperties["CODE"]=="EMAIL")):?>class="email" <?endif;?>type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>">
						<?
					}
					...
					elseif ($arProperties["TYPE"] == "LOCATION")
					{
						...
						if (is_array($arProperties["VARIANTS"]) && count($arProperties["VARIANTS"]) > 0)
						{
							...
							{
								if ($arVariant["SELECTED"] == "Y") { $value = $arVariant["ID"]; break; }
							}
						}
					...
					if (strlen($arProperties["DESCRIPTION"]) > 0){?><div class="desc"><?echo $arProperties["DESCRIPTION"] ?></div><?}?>
				</td>
			</tr>
			<?
		} return true;
...
<tr>
	...
	<select name="PROFILE_ID" id="ID_PROFILE_ID" onChange="SetContact(this.value)">
		...
		<?
			...
			{
				if ($arUserProfiles["CHECKED"]=="Y") $bHideProps = true;
...
<div class="title">
	...
	<?if ($bHideProps && $_POST["showProps"] != "Y"):?>
		<a href="#" onClick="fGetBuyerProps(this);return false;" class="pseudo"><?=GetMessage('SOA_TEMPL_BUYER_SHOW');?></a>
	<?elseif ($bHideProps && $_POST["showProps"] == "Y"):?>
		<a href="#" onClick="fGetBuyerProps(this);return false;" class="pseudo"><?=GetMessage('SOA_TEMPL_BUYER_HIDE');?></a>
	<?endif;?>
