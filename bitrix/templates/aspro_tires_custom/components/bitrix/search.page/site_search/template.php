...
<?endif;?>
	&nbsp;<button type="submit" value="<?=GetMessage("SEARCH_GO")?>" class="button25 grey"><?=GetMessage("SEARCH_GO")?></button>
...
<?
	...
	{
		$findedCount = $arResult["NAV_RESULT"]->SelectedRowsCount();
	}
?>
<?if( intval($findedCount) > 0):?>
	<div class="result-block">		
		<?=declOfNum($findedCount, array(GetMessage("FINDED_1"), GetMessage("FINDED_2"), GetMessage("FINDED_2")))?>
		<span><?=$findedCount;?></span>
		<?=declOfNum($findedCount, array(GetMessage("PRODUCTS_1"), GetMessage("PRODUCTS_2"), GetMessage("PRODUCTS_3")))?>
	</div>
<?endif;?>
	...
	<?foreach($arResult["SEARCH"] as $key => $arItem):?>
		<div class="item">
			...
			<?if($arItem["CHAIN_PATH"]):?>
				<div class="module-breadcrumbs"><?=GetMessage("SEARCH_PATH")?><?=$arItem["CHAIN_PATH"]?></div>
			<?endif;?>
		</div>
		<hr />
			
		<?if (!$arResult["SEARCH"][$key+1]):?>
