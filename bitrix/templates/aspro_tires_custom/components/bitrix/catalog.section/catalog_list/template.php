		...
		<thead>
			<tr class="colored">
				<?
					...
					if (( count($arResult["ITEMS"])>1) || ( (count($arResult["ITEMS"])==1)&&(key($arResult["ITEMS"])!="NO_DIAMETER") ) ) 
					{ $colspan=3;} else {$colspan=2;}
				?>
										...
										<div class="cost">
											...
											<?if( $arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"] ){?>
												...
												<div class="prompt-discont">
													<span>&nbsp;<strike><span><?=$arPrice["PRINT_VALUE"]?></span></strike>
												</div>
											<?}else{?>
												<div class="price_value">
													<?
...
<?}else{?>
	...
	<?
		...
		{
			$db_res = CIBlock::GetByID($arParams["IBLOCK_ID"]);
			if($iblock = $db_res->GetNext())  { $IBlockName = $iblock['NAME']; }
		}
	?>
	<?if ($IBlockName):?><h2 class="headerh2 no_products"><?=$IBlockName;?>:</h2><?endif;?>
